...
void SM4_KeySchedule(const unsigned char MK[], unsigned int rk[]){
    ...
    for(i = 0; i < 4; ++i){
        K[i] = SM4_FK[i] ^ ((MK[4*i] << 24u) | (MK[4*i+1] << 16u)
                | (MK[4*i+2] << 8u) | (MK[4*i+3]) );
    }
    for(i = 0; i < 32; ++i){
...
void SM4_Encrypt(unsigned char MK[], const unsigned char PlainText[], unsigned char CipherText[]){
    ...
    for(j = 0; j < 4; j++){
        X[j] = (PlainText[j*4] << 24u) | (PlainText[j*4+1] << 16u)
             | (PlainText[j*4+2] << 8u) | (PlainText[j*4+3]);
    }
    for(i = 0; i < 32; i++) {
    ...
    int temp = 0;
    for(j = 0; j < 4; j++){
        //temp += sprintf_s(CipherText + temp, 3, "%02x", (X[35-j] >> 24u) & 0xFFu);
        //temp += sprintf_s(CipherText + temp, 3, "%02x", (X[35-j] >> 16u) & 0xFFu);
        //temp += sprintf_s(CipherText + temp, 3, "%02x", (X[35-j] >> 8u) & 0xFFu);
        //temp += sprintf_s(CipherText + temp, 3, "%02x", (X[35-j]) & 0xFFu);
        //printf("%s\n", CipherText);
